...
assert.match(app, /const APP_VERSION='1\.9\.5'/);
assert.match(html, /v1\.9\.5/);
assert.match(worker, /sis-gastos-v1\.9\.5/);
assert.equal(JSON.parse(version).version, '1.9.5');
...
assert.doesNotMatch(app, /totals\(\)\.balance<0/);
assert.doesNotMatch(app, /Los gastos superan los fondos disponibles/);
...
console.log('Controles estáticos v1.9.5: OK');
