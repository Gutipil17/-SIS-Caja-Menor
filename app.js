...
function drawScof(doc,all,logo){
  addHeader(doc,'CAJA MENOR - SCOF01',logo,1);const {w,h}=pageSize(doc);const m=state.meta,t=totals();
  doc.setFontSize(7.5);const left=[['LUGAR Y FECHA',m.placeDate],['PERIODO',m.period],['TARJETA PEOPLE PASS',m.cardNumber],['SALDO INICIAL',money(m.initialBalance)],['2DO DEPÓSITO',money(m.secondDeposit)],['GASTOS CAJA MENOR',money(t.spent)],['SALDO FINAL',money(t.balance)]],right=[['NOMBRE RESPONSABLE',m.responsible],['ÁREA',m.area],['CARGO',m.position],['AERONAVE',m.aircraft]];
...
function drawReceipt(doc,x,y,w,h,item,num,logo){
  ...
  const top=y+23,row1=12,row2=12,conceptH=Math.max(19,h*.23),lettersH=Math.max(17,h*.19),bottomH=22;const c1=x+w*.40,c2=x+w*.50,c3=x+w*.60,c4=x+w*.74;
  doc.rect(x,top,w,row1);[c1,c2,c3,c4].forEach(xx=>doc.line(xx,top,xx,top+row1));doc.setFont('helvetica','bold');doc.setFontSize(6.8);doc.text('CIUDAD',x+3,top+4);doc.text('DÍA',c1+2,top+4);doc.text('MES',c2+2,top+4);doc.text('AÑO',c3+2,top+4);doc.text('No.',c4+2,top+4);const d=new Date(item.date+'T00:00:00');fit(doc,item.city,x+3,top+9,c1-x-6,1,8,'bold');doc.setFont('helvetica','bold');doc.setFontSize(8);doc.text(String(d.getDate()).padStart(2,'0'),c1+7,top+9);doc.text(String(d.getMonth()+1).padStart(2,'0'),c2+7,top+9);doc.text(String(d.getFullYear()),c3+6,top+9);doc.text(`RC-${String(num).padStart(3,'0')}`,c4+4,top+9);
  let yy=top+row1;doc.rect(x,yy,w,row2);doc.line(x+w*.72,yy,x+w*.72,yy+row2);doc.setFont('helvetica','bold');doc.setFontSize(6.8);doc.text('PAGADO A',x+3,yy+4);doc.text('$',x+w*.74,yy+8.5);fit(doc,item.thirdParty,x+3,yy+9.5,w*.67,1,8.5,'bold');doc.setFont('helvetica','bold');doc.setFontSize(9);doc.text(new Intl.NumberFormat('es-CO').format(item.amount),x+w-4,yy+8.7,{align:'right'});
  yy+=row2;doc.rect(x,yy,w,conceptH);doc.setFont('helvetica','bold');doc.setFontSize(6.8);doc.text('CONCEPTO',x+3,yy+4.5);fit(doc,item.detail||item.category,x+3,yy+10,w-6,3,8.2,'bold');
  yy+=conceptH;doc.setFillColor(...pale);doc.rect(x,yy,w,lettersH,'FD');doc.setFont('helvetica','bold');doc.setFontSize(6.8);doc.text('VALOR (EN LETRAS)',x+3,yy+4.5);fit(doc,`${words(item.amount)} PESOS M/CTE`,x+3,yy+10,w-6,2,7.8,'bold');
  yy+=lettersH;doc.rect(x,yy,w,bottomH);const leftW=w*.34;doc.line(x+leftW,yy,x+leftW,yy+bottomH);doc.line(x,yy+bottomH/2,x+leftW,yy+bottomH/2);doc.setFont('helvetica','bold');doc.setFontSize(6.5);doc.text('CÓDIGO',x+3,yy+5);doc.text('APROBADO',x+3,yy+bottomH/2+5);doc.text('FIRMA DE RECIBIDO',x+leftW+3,yy+5);const sigY=yy+bottomH-6;if(item.signature)doc.addImage(item.signature,'PNG',x+leftW+28,yy+2,w-leftW-34,bottomH-8);doc.line(x+leftW+3,sigY,x+w-3,sigY);doc.setFont('helvetica','bold');doc.setFontSize(6.2);const isNit=/nit|rut/i.test(item.idType||'');doc.text(isNit?'NIT:':'C.C.:',x+leftW+3,yy+bottomH-2);fit(doc,item.idNumber,x+leftW+19,yy+bottomH-2,w-leftW-23,1,7,'bold')
}
